...
const TOKEN_MAP = {
  ...
  // Nested tokens for dynamic content (e.g. dropdown options)
  "##accFname##": "{{recipient.firstname}}",
  "##accLname##": "{{recipient.lastname}}",
  "##accTitle##": "{{recipient.title}}",
  "##User.FirstName##": "{{sender.firstname}}",
...
/**
 * Apply all build transformations to an HTML string.
 */
function transformHtml(html) {
  ...
  return { finalHtml, stats, dropdowns, maxLenTokens, preheaderCleared, customTextNumbers, fragments, imagePaths };
}

/**
 * Recursively collect all .html files under `dir`, excluding *_lsc.html files.
 */
function collectHtmlFiles(dir) {
  ...
  for (const entry of fs.readdirSync(dir, { withFileTypes: true })) {
    ...
    } else if (entry.isFile() && path.extname(entry.name).toLowerCase() === ".html" && !entry.name.endsWith("_lsc.html")) {
...
/**
 * Process all HTML files in the fragments folder and write *_lsc.html next to each.
 */
function processFragments() {
  if (!fs.existsSync(FRAGMENTS_DIR)) {
    console.log("ℹ️  No fragments/ folder found, skipping.");
    return;
  }

  const files = collectHtmlFiles(FRAGMENTS_DIR);
  if (files.length === 0) {
    console.log("ℹ️  No HTML files found in fragments/, skipping.");
    return;
  }

  console.log(`\n-- Fragment builds (${files.length} file(s)) --`);
  for (const filePath of files) {
    const html = fs.readFileSync(filePath, "utf8");
    const { finalHtml } = transformHtml(html);
    const ext    = path.extname(filePath);
    const base   = path.basename(filePath, ext);
    const outPath = path.join(path.dirname(filePath), `${base}_lsc${ext}`);
    fs.writeFileSync(outPath, finalHtml, "utf8");
    console.log(`  ✅  ${path.relative(SRC_DIR, filePath)}  →  ${path.relative(SRC_DIR, outPath)}`);
  }
  console.log("--\n");
}
...
async function main() {
  ...
  const { finalHtml, stats, dropdowns, maxLenTokens, preheaderCleared, customTextNumbers, fragments, imagePaths } = transformHtml(html);
